...
const sourceRoot =
  process.argv[2] ||
  path.join("《大李敖全集6.0》分章节", "001.自传回忆类", "007.李敖风流自传");
const candidatePath =
  process.argv[3] || path.join("analysis", "liao_romantic_autobio_quote_candidates.json");

const outKeywordPath = path.join("analysis", "liao_romantic_autobio_keyword_lines.tsv");
const outKeywordShortPath = path.join("analysis", "liao_romantic_autobio_keyword_lines_short.tsv");
const outPlausiblePath = path.join("analysis", "liao_romantic_autobio_plausible_quotes.tsv");
const outKnownMatchesPath = path.join("analysis", "liao_romantic_autobio_known_quote_matches.tsv");

const sourceDecoder = new TextDecoder("gb18030");

const keywordPattern =
  /诗说|杜甫|李白|有诗|题诗|妙诗|打油诗|古话|俗话|俗语|古语|谚|成语|名言|名句|格言|对联|联语|所谓|曰|云|孔子|孟子|论语|庄子|老子|韩非子|金刚经|圣经|莎士比亚|歌德|亚里士多德|王安石|苏轼|苏东坡|陆游|龚定盦|顾亭林|鲁迅|胡适|王闿运|唱|歌词|儿歌|歌谣|俗以|世称/;
const quoteContextPattern =
  /诗|词|曰|云|所谓|古人|孔子|孟子|庄子|老子|鲁迅|胡适|歌德|莎士比亚|谚|俗话|俗语|古语|成语|名言|格言|名句|联|对联|题词|诗云|语曰|说过|有言|箴|歌|王安石|杜甫|李白|苏轼|苏东坡|龚定盦|顾亭林|王闿运|金刚经|圣经/;
...
for (const row of candidates) {
  ...
  if (length < 4 || length > 100) continue;
