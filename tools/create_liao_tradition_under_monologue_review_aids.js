...
const sourceRoot =
  process.argv[2] ||
  path.join("《大李敖全集6.0》分章节", "002.精品散文类", "003.独白下的传统");
const candidatePath =
  process.argv[3] || path.join("analysis", "liao_tradition_under_monologue_quote_candidates.json");

const outKeywordPath = path.join("analysis", "liao_tradition_under_monologue_keyword_lines.tsv");
const outKeywordShortPath = path.join(
  "analysis",
  "liao_tradition_under_monologue_keyword_lines_short.tsv",
);
const outPlausiblePath = path.join("analysis", "liao_tradition_under_monologue_plausible_quotes.tsv");
const outKnownMatchesPath = path.join(
  "analysis",
  "liao_tradition_under_monologue_known_quote_matches.tsv",
);

const sourceDecoder = new TextDecoder("gb18030");

const keywordPattern =
  /诗说|有诗|诗云|诗曰|诗|词|歌|曲|唱|歌词|歌谣|童谣|谣|谚|俗话|俗语|古语|古话|成语|名言|名句|格言|联|对联|联语|所谓|曰|云|说过|有言|箴|孔子|孟子|庄子|老子|荀子|韩非子|墨子|论语|孟子|庄子|老子|礼记|大学|中庸|诗经|尚书|易经|周易|春秋|左传|国语|史记|汉书|后汉书|资治通鉴|世说新语|水浒传|红楼梦|三国演义|金瓶梅|佛经|金刚经|圣经|杜甫|李白|白居易|苏轼|苏东坡|王安石|欧阳修|陆游|龚定盦|龚自珍|顾亭林|鲁迅|胡适|莎士比亚|歌德|尼采|叔本华|柏拉图|亚里士多德/;
const quoteContextPattern =
  /诗|词|歌|曲|谣|谚|俗话|俗语|古语|古话|成语|名言|名句|格言|联|对联|联语|所谓|曰|云|说过|有言|箴|孔子|孟子|庄子|老子|荀子|韩非子|墨子|论语|礼记|诗经|尚书|易经|左传|史记|汉书|后汉书|资治通鉴|世说新语|红楼梦|水浒传|三国演义|佛经|金刚经|圣经|杜甫|李白|白居易|苏轼|王安石|陆游|鲁迅|胡适|莎士比亚|歌德/;
