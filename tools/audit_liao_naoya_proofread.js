...
const bookCsv = path.join("exports", "李敖闹衙集_诗文格言歌谣引用.csv");
const totalCsv = path.join("exports", "总_诗文格言歌谣引用.csv");
const outTsv = path.join("analysis", "liao_naoya_proofread_lexicon_hits.tsv");
...
function excerpt(value, size = 220) {
...
const selected = new Set(parseCsv(fs.readFileSync(bookCsv, "utf8")).map((row) => row.quote_text));
const totalRows = parseCsv(fs.readFileSync(totalCsv, "utf8"));
const lexicon = [
  ...new Set(
    totalRows
      .map((row) => row.quote_text)
      .filter((text) => text && !selected.has(text))
      .filter((text) => {
